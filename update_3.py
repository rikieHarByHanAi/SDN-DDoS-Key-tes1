...
progress_file = 'cross_validation_progress.pkl'

# Cek apakah ada progres yang sudah disimpan
if os.path.exists(progress_file):
    with open(progress_file, 'rb') as f:
        saved_data = pickle.load(f)
    accuracies = saved_data['accuracies']
    precisions = saved_data['precisions']
    recalls = saved_data['recalls']
    f1_scores = saved_data['f1_scores']
    histories = saved_data['histories']
    fprs = saved_data['fprs']
    tprs = saved_data['tprs']
    aucs = saved_data['aucs']
    cms = saved_data['cms']
    attention_weights_all_folds = saved_data['attention_weights_all_folds']
    start_fold = len(accuracies)  # Mulai dari fold berikutnya
    print(f"Melanjutkan dari fold {start_fold + 1}")
else:
    start_fold = 0
...
for fold, (train_index, test_index) in enumerate(kf.split(X_seq)):
    if fold < start_fold:
        continue  # Lewati fold yang sudah selesai
    ...
    progress_data = {
        ...
        'aucs': aucs,
        'cms': cms,
        'attention_weights_all_folds': attention_weights_all_folds
    }
...
# Hapus file progres setelah selesai
if os.path.exists(progress_file):
