...
# Reshape data untuk LSTM dan CNN (seq_length=10)
seq_length = 10
X_seq = []
y_seq = []
...
# Flatten data untuk Random Forest
X_flat = X_seq.reshape(X_seq.shape[0], -1)  # (n_samples, seq_length * n_features)
...
for fold, (train_index, test_index) in enumerate(kf.split(X_seq)):
    ...
    X_train_flat, X_test_flat = X_flat[train_index], X_flat[test_index]
...
fig.suptitle('Comparison of Model Performance (5-Fold Cross-Validation)', fontsize=16)
